...
public class Treillis {
    ...
    public NoeudSimple demandeNoeudSimple() {
        ...
        while (x < this.terrain.getXmin() || x > this.terrain.getXmax()) {
            System.out.println("Donner une nouvelle abcisse pour être dans la zone constructible : ");
            x = Lire.d();
        }
        System.out.println("ordonné : ");
        double y = Lire.d();
        while (y < this.terrain.getXmin() || y > this.terrain.getXmax()) {
            System.out.println("Donner une nouvelle ordonnée pour être dans la zone constructible : ");
            y = Lire.d();
        }
    ...
    public Barre creationBarre() {
        ...
        while (l < tBarre.getLmin() || l > tBarre.getLmax() || l == -1) {
            System.out.println("La longueur de la barre ne correspond pas à la longueur de ce type de barre, choisir a nouveau taper (-1 pour quitter) ");
            if (Lire.i() == -1) {
                return null;
            } else {
                System.out.println("Choix du premier noeud : \n");
    ...
    public TriangleTerrain creationTriangleTerrainAPartirDeRien() {
        ...
        p1.setNum(num1);
        System.out.println("Donner le second point (num différent du premier) : ");
        p2 = p2.demandePoint();
        int num2 = p2.getNum();
        System.out.println(num2);
        while (num2 == p1.getNum()) {//num2 < 0 || num2 > 2
            System.out.println("Donner un num correct (différent du premier et compris entre 0 et 2) :");
            num2 = Lire.i();
        }
        p2.setNum(num2);
    ...
    public TriangleTerrain creationTriangleTerrainTriangleDejaExistant() {
        ...
        System.out.println("Numéro du point (compris entre 0 et 2) : ");
        int num1 = Lire.i();
        while (num1 < 0 || num1 > 2) {
            System.out.println("Donner un numéro compris entre 0 et 2 :");
            num1 = Lire.i();
        }
        p1.setNum(num1);
        ...
        System.out.println("Numéro du point ( entre 0 et 2, et différent du premier num) : ");
        int num2 = Lire.i();
        while (num2 < 0 || num2 > 2 || num2 == num1) {
            System.out.println("Donner un numéro compris entre 0 et 2 ( différent du premier numéro) :");
            num2 = Lire.i();
        }
        p2.setNum(num2);
        System.out.println("Donner le troisième point : ");
    ...
    public NoeudAppuiSimple creationNoeudAppuiSimple() {
        ...
        NoeudAppuiSimple res = new NoeudAppuiSimple(0, alpha, t1, p1.getNum());// attention choix numéro premier point segment terrain
        Point p = new Point(0, 0);

        p = coordonneNoeudAppui(res);
        if (p.getPx() < this.terrain.getXmin() || p.getPx() > this.terrain.getXmax()) {
            System.out.println("Création noeud appui impossible (hors zone construction) ");
            return null;
    ...
    public void menuTexte() {
        ...
        while (rep != 0) {
            ...
            System.out.println("8) Vérifier si un point est dans un triangle");
